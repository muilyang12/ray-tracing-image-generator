...
import { useModelDataStore } from "../stores/useModelDataStore";
...
function ObjViewer({ file }: ObjViewerProps) {
  ...
  const setModelCenter = useModelDataStore((state) => state.setModelCenter);
  const setBoundingSphereRadius = useModelDataStore((state) => state.setBoundingSphereRadius);
  ...
  const loadObj = useCallback((file: File) => {
    ...
    fileReader.onload = () => {
      ...
      setModelCenter(boundingSphere.center);
